...
<script type="text/javascript">
	window.onload = function() {
		...
		var selTr; // 선택한 행을 바라볼 변수
		var arr; // 선택한 행의 자신 노드(td들)를 바라볼 변수(배열)
		
		// 삭제버튼이 눌러졌을 떄의 이벤트 처리
		...
		// 수정버튼이 눌러졌을 때의 이벤트 처리
		document.getElementById("btnUpdate").onclick = function() {
			
			// 총점을 누적하기 위한 변수를 선언한다.
			var tot = 0;
			
			// input태그의 수 만큼 반복 실행한다.
			for (var i = 0; i < input.length; i++) {
				...
				if (i != 0) {
					...
					// 입력양식의 내용은 문자로 취급하므로
					// eval을 이용하여 계산할 수 있는 형태로 변경하여 누적한다.
			...
			// 계산된 총점과 평균을 선택한 행의 총점과 평균을 출력하기 위한
			// td의 내용으로 설정한다.
			arr[4].innerHTML = tot;
			arr[5].innerHTML = avg;
		}
		
		// 등록버튼이 눌러졌을 때의 이벤트 처리
		document.getElementById("btnAdd").onclick = function() {
			...
			tr.onclick = function() {
				...
				// 선택한 tr의 자식 노드를 읽어 옵니다. 그것은 곧 td들입니다.
				arr = this.childNodes;
			...
			for(var i = 0; i < input.length; i++ ) {
				...
				// 총점을 누적한다.
				// i가 0일 때는 이름이므로 제외하고 누적한다.
				if (i != 0) {
					
					// 계산할 수 있는 숫자로 변환하여 누적한다.
					tot += eval(input[i].value);
			...
			// 총점을 위한 td태그에 텍스트 노드를 담는다.
			tdTot.appendChild(txtTot);
			
			// 평균을 위한 td태그에 텍스트 노드를 담는다.
